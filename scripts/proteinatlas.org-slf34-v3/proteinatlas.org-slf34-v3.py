...
filename = 'proteinatlas.org-slf34-v3.pkl'
if not os.path.isfile( filename ):
  print "Content database not found on disk. Downloading file."
  murphylab = 'http://murphylab.web.cmu.edu/software/searcher/Databases/' + filename
  url = urllib.urlretrieve( murphylab, filename )
else:
  print "Content database found on disk. Loading from existing file."

output_directory = 'cypher_files'
if not os.path.exists( output_directory ):
  print "Output folder not found. Making folder."
  os.makedirs( output_directory )
else:
  print "Output folder found."

print "Loading content database"
database = pickle.load( file( filename, 'r' ) )
...
for key in keys:
  if key != 'info':
    resolution = key
    print "Building Cypher file for resolution " + str(resolution)

    output_filename = 'proteinatlas.org-slf34-resolution' + str(resolution) + '.cql'
    print "Setting Cypher file to " + output_filename
    file = open( output_filename, 'w' )

    print "Processing database" + str(resolution)
    data = database[key]

    for datum in data:
      print "Processing image with image identification:" + str(datum[0])
      string = ""
      string = string + "begin\n"
      string = string + "CREATE (n:Image {" 
      string = string + "resolution:" +  str(resolution) + ","
      for i in range(10):
        feature = datum[i]
        if feature == 0:
          feature = int(feature)
        ...
        if isinstance( feature, str ):
          string = string + 'temp' + str(i) + ":'" + str(datum[i]) + "',"
        else:
          string = string + 'temp' + str(i) + ":" + str(datum[i]) + ","

      for i in range(10,len(datum),1):
        feature = datum[i]
        if isinstance(feature, (float,long,int)) and feature == 0:
          feature = int(feature)

        if isinstance( feature, str ):
          string = string + feature_ids[i-10].replace('.','_') + ":'" + str(datum[i]) + "',"  
        elif isinstance( feature, float ):
          string = string + feature_ids[i-10].replace('.','_') + ":" + '{0:.15f}'.format(datum[i]) + ","  
        else:
          string = string + feature_ids[i-10].replace('.','_') + ":" + str(datum[i]) + ","  
